...
def query_user_range(name: str, ts_start: int = None, ts_end: int = None, limit: int = None, scan_forward: bool = True):
    """
...
def user_entries(name):
    ...
    out = [{"ts": int(it["ts"]), "text": censor(safe_text(it.get("text", "")))} for it in items]
...
def all_recent():
    ...
    out = [{"user": censor(it.get("user", "")),
            "ts": int(it["ts"]),
            "text": censor(safe_text(it.get("text", "")))} for it in items]
    return jsonify({"entries": out})
